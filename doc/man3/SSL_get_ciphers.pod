...
SSL_CTX_get_ciphers() returns an internal stack owned by B<ctx>; its elements
remain valid while B<ctx> remains alive.
SSL_get_ciphers() and SSL_get_client_ciphers() return internal stacks whose
elements remain valid while B<ssl> remains alive. The caller B<MUST NOT> free
these stacks or their elements.
