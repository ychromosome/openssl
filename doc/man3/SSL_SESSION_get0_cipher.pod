...
should not be released. It remains valid for the lifetime of B<s>, including
for provider-defined ciphersuites after the connection and SSL_CTX are freed.
...
It remains eligible for explicit use through the TLS 1.3 external-PSK
callbacks, including for 0-RTT. Such use does not lift any of these
restrictions. A successful d2i_SSL_SESSION() call replaces the logical session
with the decoded built-in session state.
