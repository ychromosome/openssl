...
Provider-defined ciphersuite wire IDs resolve through the connection's
original SSL_CTX. If the callback switches contexts, the active context must
expose the same fetched cipher and digest implementations with equivalent
metadata, and the suite must remain enabled in the connection's ciphersuite
list. Its security callback must also permit the suite. SSL_set_SSL_CTX()
rejects an incompatible target before changing the context. The same rule
applies to context switches from later callbacks. During a handshake, an
incompatible switch makes the handshake fail even if the callback ignores the
return value.
