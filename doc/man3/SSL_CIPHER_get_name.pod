...
For provider-defined TLS 1.3 ciphersuites, SSL_CIPHER_get_name() and
SSL_CIPHER_standard_name() return the capability name, which need not be
IANA-allocated. OPENSSL_cipher_name() searches built-in ciphersuites only.
...
for AEAD cipher suites) then B<NID_undef> is returned. For a provider-defined
TLS 1.3 ciphersuite it returns the handshake digest NID.
...
may use private-use values. This parameter is usually retrieved from a TLS
packet by using functions like
