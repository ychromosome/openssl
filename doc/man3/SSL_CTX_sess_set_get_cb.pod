...
For a session that has used a provider-defined ciphersuite,
new_session_cb() is informational. The session remains non-resumable,
non-cacheable, non-serialisable, and ticket-free.
