...
SSL_set_SSL_CTX() keeps its existing behavior: an SSL object without its own
cipher list follows the cipher list of its current SSL_CTX, while a list set
with SSL_set_cipher_list() or SSL_set_ciphersuites() persists across the
switch. Provider descriptors continue to resolve through the connection's
original SSL_CTX. Switching to a context with a different library context or
property query is outside this capability's contract.
