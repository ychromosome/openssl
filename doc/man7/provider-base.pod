...
After one is selected or installed, SSL_set_SSL_CTX() requires an equivalent
suite in the target context and approval from its security callback.
...
may supply a session with such a suite through the stream TLS 1.3 external-PSK
callbacks, including for 0-RTT. Provider-suite 0-RTT requires an equivalent
enabled descriptor under the connection policy; otherwise early data is
suppressed or rejected. DTLS and QUIC reject such sessions. This does not make
that session eligible for
ordinary resumption, caching, serialisation or tickets.
