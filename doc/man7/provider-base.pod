...
A nonzero 16-bit TLS value. GREASE and duplicate values are rejected.

=item "tls-ciphersuite-aead-name" (B<OSSL_CAPABILITY_TLS_CIPHERSUITE_AEAD>) <UTF8 string>

An AEAD fetch name. The cipher must have a fixed positive key length no greater
than B<EVP_MAX_KEY_LENGTH>, a 12-byte IV, a one-byte block size and a 16-byte
default tag. CCM is rejected.
...
The AEAD and digest are fetched with the SSL_CTX library context and property
query and retained by the descriptor. Discovery occurs at SSL_CTX creation;
the registry is then immutable. Unsupported and empty capabilities are ignored.
Malformed or aborted nonempty capabilities fail SSL_CTX creation.
