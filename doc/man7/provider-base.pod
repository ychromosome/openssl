...
The "TLS-CIPHERSUITE" capability can be queried by libssl to discover
additional TLS 1.3 ciphersuites. Providers that supply this capability describe
each ciphersuite by calling the callback passed to provider_get_capabilities().
The following parameters are mandatory. Unknown parameters are ignored.

=over 4

=item "tls-ciphersuite-name" (B<OSSL_CAPABILITY_TLS_CIPHERSUITE_NAME>) <UTF8 string>

The ciphersuite name. It must be a nonempty printable ASCII string of at most
255 bytes and must not contain a colon. The name must not collide, ignoring
case, with a built-in or another provider-defined ciphersuite in the same
library context.

=item "tls-ciphersuite-code-point" (B<OSSL_CAPABILITY_TLS_CIPHERSUITE_CODE_POINT>) <unsigned integer>

The two-byte TLS ciphersuite code point. Zero, GREASE values, built-in code
points, and duplicate provider-defined code points are rejected.

=item "tls-ciphersuite-aead-name" (B<OSSL_CAPABILITY_TLS_CIPHERSUITE_AEAD>) <UTF8 string>

The name of an AEAD cipher implementation fetchable with the library context
and property query of the L<SSL_CTX_new_ex(3)> call. The fetched cipher must
have a positive fixed key length, a 12-byte IV, a one-byte block size and a
16-byte default authentication tag. CCM mode is not supported. The fetched
object is retained for the lifetime of the ciphersuite descriptor.

=item "tls-ciphersuite-digest-name" (B<OSSL_CAPABILITY_TLS_CIPHERSUITE_DIGEST>) <UTF8 string>

The name of the TLS 1.3 transcript digest, fetched using the same library
context and property query. It must identify SHA2-256 or SHA2-384. The fetched
object is retained for the lifetime of the ciphersuite descriptor.

=item "tls-ciphersuite-security-bits" (B<OSSL_CAPABILITY_TLS_CIPHERSUITE_SECURITY_BITS>) <unsigned integer>

The security strength of the ciphersuite. It must be at least 128 bits and no
greater than the fetched AEAD key size.

=back

Provider-defined ciphersuites are discovered when an SSL_CTX is created and
the resulting registry is immutable for that SSL_CTX. Loading another provider
afterwards does not add ciphersuites to an existing SSL_CTX. A provider that
does not implement this capability is ignored, as is a provider that reports an
empty capability. Once a provider has begun supplying descriptors, a malformed
descriptor or an aborted enumeration causes SSL_CTX creation to fail.

Provider-defined ciphersuites are not added to the default ciphersuite list and
must be selected by name. They are restricted to TLS 1.3 and are not available
for DTLS or QUIC. Sessions that have used a provider-defined ciphersuite are not
resumable, cacheable or serialisable and do not produce session tickets.
