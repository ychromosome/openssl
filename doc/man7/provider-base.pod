...
=head3 "TLS-CIPHERSUITE" Capability

The "TLS-CIPHERSUITE" capability adds TLS 1.3 ciphersuites to libssl. Each
callback describes one ciphersuite. All parameters below are mandatory;
unknown parameters are ignored.

=over 4

=item "tls-ciphersuite-name" (B<OSSL_CAPABILITY_TLS_CIPHERSUITE_NAME>) <UTF8 string>

A name of 1 to 255 bytes. Each byte must be in the range 0x21 through 0x7e,
excluding colon. It must be unique, ignoring case, among the OpenSSL and
standard names of built-in ciphersuites and provider-defined names. The
parameter data size must describe exactly the string bytes, with or without
one terminating NUL; embedded NULs and trailing bytes are rejected.

=item "tls-ciphersuite-code-point" (B<OSSL_CAPABILITY_TLS_CIPHERSUITE_CODE_POINT>) <unsigned integer>

A nonzero 16-bit TLS value that is not a GREASE value and does not collide
with a built-in or previously discovered ciphersuite. Libssl does not check
IANA allocation status. Use a Private Use value unless the advertised suite
has an assigned code point.

=item "tls-ciphersuite-aead-name" (B<OSSL_CAPABILITY_TLS_CIPHERSUITE_AEAD>) <UTF8 string>

An AEAD fetch name of 1 to 255 bytes. The cipher must be non-CCM and report a
positive key length no greater than B<EVP_MAX_KEY_LENGTH>, a 12-byte IV and a
one-byte block size. Libssl derives and supplies the reported key length;
discovery does not prove that the cipher rejects other lengths. AES-128-GCM
must report 16 bytes. AES-256-GCM and ChaCha20-Poly1305 must report 32 bytes.
The cipher must support the TLS 1.3 record operations used by libssl:
setting the IV length and authentication tag, obtaining the authentication
tag, AAD updates, and in-place encryption and decryption.

=item "tls-ciphersuite-digest-name" (B<OSSL_CAPABILITY_TLS_CIPHERSUITE_DIGEST>) <UTF8 string>

A TLS 1.3 transcript digest fetch name of 1 to 255 bytes: SHA2-256 with a
32-byte output or SHA2-384 with a 48-byte output.

=item "tls-ciphersuite-security-bits" (B<OSSL_CAPABILITY_TLS_CIPHERSUITE_SECURITY_BITS>) <unsigned integer>

At least 128 and no greater than the AEAD key size in bits.

=item "tls-ciphersuite-tag-length" (B<OSSL_CAPABILITY_TLS_CIPHERSUITE_TAG_LENGTH>) <unsigned integer>

The record authentication tag length. It must be 16.

Unsigned integer parameters must use between 1 and 8 data bytes.

=back

The AEAD and digest are fetched using the SSL_CTX library context and property
query. They need not be implemented by the provider advertising the
ciphersuite. The fetched objects are retained by the descriptor.
Discovery occurs at SSL_CTX creation; the registry is then immutable.
Providers that do not implement the capability, supported empty capabilities,
and entries whose AEAD or digest cannot be fetched are ignored. Malformed
entries, collisions, profile violations, and providers that abort after
supplying an entry cause context creation to fail. Libssl allocation and
registry failures also abort context creation. An SSL_CTX accepts at most 128
provider-defined ciphersuites.

A return of 0 before the first callback is treated as an unsupported
capability. A return of 0 after a callback aborts context creation.

Provider-defined ciphersuites are not added to the default ciphersuite list and
must be selected by name. Their code points require deployment coordination.
SSL_set_SSL_CTX() keeps its existing behavior; provider descriptors continue
to resolve through the connection's original SSL_CTX. Switching to a context
with a different library context or property query is outside this capability's
contract.
They are restricted to TLS 1.3 and are not available for DTLS or QUIC.
DTLS 1.3 record-number encryption and QUIC header protection require a
companion algorithm that this capability does not declare.
They have no built-in cipher-family classification, so family-specific
preferences do not apply. Provider-defined ciphersuites do not use kTLS.
Sessions that have used a provider-defined ciphersuite are not resumable,
cacheable or serialisable and do not produce session tickets. An application
cannot supply such a session through external-PSK callbacks. Provider-backed
external PSK and 0-RTT are not supported. A selected provider-defined
ciphersuite accepts neither tickets nor external PSKs, including built-in
sessions with a compatible transcript digest. A server ignores such PSK
offers and continues with a full handshake. A client rejects a server that
selects a PSK together with a provider-defined ciphersuite.

This capability supports the existing stream TLS 1.3 record profile: a
12-byte IV, 16-byte tag, and SHA-256 or SHA-384 transcript digest. Discovery
validates metadata only. Successful context creation does not guarantee that
record setup or the first record operation will succeed.

Libssl permits at most 2^24 encrypted records under one provider-suite traffic
key. KeyUpdate installs a new traffic key and resets this count. An AEAD with a
lower algorithm-specific limit must refuse encryption before that limit;
cipher failure is fatal to the record layer.
...
A return of 0 before the first callback is treated as an unsupported
capability. A callback failure or a return of 0 after a callback aborts context
creation.
