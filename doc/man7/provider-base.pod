...
IANA allocation status. A non-private deployment must use a code point
assigned for the advertised suite.

=item "tls-ciphersuite-aead-name" (B<OSSL_CAPABILITY_TLS_CIPHERSUITE_AEAD>) <UTF8 string>

An AEAD fetch name. The cipher must be non-CCM and have a fixed positive key
length no greater than B<EVP_MAX_KEY_LENGTH>, a 12-byte IV and a one-byte
block size. It must support the TLS 1.3 record operations used by libssl:
...
A TLS 1.3 transcript digest fetch name: SHA2-256 or SHA2-384.
...
preferences and kTLS recognition do not apply.
...
may supply a session with such a suite through the TLS 1.3 external-PSK
callbacks, including for 0-RTT. This does not make that session eligible for
ordinary resumption, caching, serialisation or tickets.

Discovery validates metadata only. It does not execute record operations and
does not enforce provider-specific record usage limits.
