...
discovery does not prove that the cipher rejects other lengths. The cipher
must support the TLS 1.3 record operations used by libssl:
...
Existing providers return 0 both for unsupported capabilities and for errors.
Libssl therefore treats an outer capability failure as unsupported and
discards descriptors and errors produced by that provider query. A callback
validation or allocation failure remains fatal. Entries whose AEAD or digest
fetch fails are skipped; fetch failures are not classified. Malformed entries,
collisions, profile violations, allocation failures and registry failures
abort context creation. An SSL_CTX accepts at most 128 provider-defined
ciphersuites.
...
SSL_set_SSL_CTX() keeps its existing behavior: it does not replace the SSL
object's saved TLS 1.3 ciphersuite list. SSL_get_ciphers() borrows the current
SSL_CTX's combined list until a per-SSL setter creates a separate list.
SSL_set_cipher_list() combines its pre-TLS-1.3 selection with the saved TLS 1.3
entries; SSL_set_ciphersuites() replaces those entries explicitly.
This capability does not add post-SNI revalidation against the selected
SSL_CTX's TLS 1.3 list. Provider descriptors continue to resolve through the
connection's original SSL_CTX. Switching to a context with a different library
context or property query is outside this capability's contract.
...
external PSK and 0-RTT are not supported.

A ticket or external PSK associated with a built-in ciphersuite may negotiate
a provider-defined suite with the same transcript hash. The resulting session
has provider-cipher provenance and cannot be resumed again. This does not
change the original ticket or externally supplied PSK session.
