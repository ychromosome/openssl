...
TLS 1.3 ciphersuite setting follows the TLS 1.3 list of its current SSL_CTX;
SSL_set_ciphersuites() makes that setting explicit and persistent. The
pre-TLS-1.3 list follows the current context unless set with
SSL_set_cipher_list(). Provider descriptors continue to resolve through the
